...
def import_urls_from_csv() -> list:
    try:
        with open('defo_urls.csv') as file:
            file_data = csv.reader(file)
            logging.info("Import urls from csv.")
            return [i[0] for i in file_data if len(i)>0]

    except FileNotFoundError:
        logging.info("Can't find a file 'defo_urls.csv'")
        print("Не смог найти файл или неверное имя - должно быть 'defo_urls.csv' ")
...
def create_price_csv(data: list[dict]):
    ...
    with open('defo_price.csv', 'w', encoding='UTF8', newline='') as file:
        ...
        for slice in output:
            for data_html in get_async_data(slice):
                if data_html:
                    ...
                    article = soup.find("span", class_="product__vendor-code vendor-code").text.replace("Артикул",
                                                                                                            "").strip()
                    price_new = soup.find("p", class_="prices__new").text.replace("руб.", "").replace(" ",
                                                                                                          "").strip()
                    price_old = soup.find("p", class_="prices__old").text.replace("руб.", "").replace(" ",
                                                                                                          "").strip()
                    data_to_file.append(
                            {"article": article,
                             "price_new": price_new,
